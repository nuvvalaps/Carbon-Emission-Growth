...
am4core.ready(function() {
  ...
  var countryIndexMap = {};
  var list = world_timeline[world_timeline.length - 1].list;
  ...
  function getSlideData(index) {
    if (index == undefined) {
      index = world_timeline.length - 1;
    }

    var data = world_timeline[index];
  ...
  title.text = "Carbon Emission Data";
  title.align = "left";
  ...
  slider.events.on("rangechanged", function(event) {
    var index = Math.round((world_timeline.length - 1) * slider.start);
  ...
  // create series
  var activeSeries = addSeries("Population", activeColor);
  // active series is visible initially
  activeSeries.tooltip.disabled = true;
  activeSeries.hidden = false;

  var confirmedSeries = addSeries("GDP", confirmedColor);
  var recoveredSeries = addSeries("Total Carbon Emission", recoveredColor);
